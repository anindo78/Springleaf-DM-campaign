...
head(strain)
# create ready copy to free memory
write.csv(strain, file="strain.csv")


# read back
train <- read.csv("strain.csv", header=T)
...
write.csv(train_int, file="shorttrain.csv")
#train_int<- read.csv("shorttrain.csv", header = T)
...
library(caret)
chrvars <- predict(dummyVars(~ ., data = train_chr), newdata = train_chr)
head(chrvars)

train_tot <- data.frame(cbind(train_int, chrvars))
write.csv(train_tot, file="train_tot.csv")

train_tot <- read.csv("train_tot.csv", header=T)
